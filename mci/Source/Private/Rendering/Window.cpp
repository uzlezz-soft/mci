...
#define DO_SOME_MAGIC(x, ...) \
    ...
    glm::vec2 Monitor::GetPhysicalSize() const
    {
        int physicalSizeX = 0;
        int physicalSizeY = 0;
        ...
        return glm::vec2((float)physicalSizeX, (float)physicalSizeY);
    }
    ...
    Monitor::VideoMode Monitor::GetVideoMode() const
    {
        ...
        return { glm::vec2((float)glfwVideoMode->width, (float)glfwVideoMode->height), (uint16)glfwVideoMode->refreshRate };
    }
    ...
    std::vector<Monitor::VideoMode> Monitor::GetAvailableVideoModes() const
    {
        ...
        for (int i = count - 1; i >= 0; i--)
        {
            ...
            videoModes.push_back({ glm::vec2((float)glfwVideoMode.width, (float)glfwVideoMode.height), (uint16)glfwVideoMode.refreshRate });
        }

        return videoModes;
    }

    glm::vec4 Monitor::GetWorkArea() const
    {
        ...
        return glm::vec4((float)workAreaPositionX, (float)workAreaPositionY, (float)workAreaSizeX, (float)workAreaSizeY);
    }
    ...
    void Window::Resize(glm::vec2 newSize) noexcept
    {
    ...
    glm::vec2 Window::GetFramebufferSize() const noexcept
    {
        int width = 0;
        int height = 0;
    ...
    void Window::SetMousePosition(glm::vec2 position) const noexcept
    {
    ...
    {
        const glm::vec2 size = GetFramebufferSize();
    ...
    void Window::OnMouseMove(glm::vec2 mousePosition)
    {
    ...
    void Window::OnMouseScroll(glm::vec2 scrollDelta)
    {
    ...
    void Window::ResetJustPressedKeys()
    {
        ...
        m_mouseMoveDelta = glm::vec2(0.0f);
        m_normalizedMouseMoveDelta = glm::vec2(0.0f);
    }
    ...
    {
        glfwInit();
        MCI_LOG_TRACE("Window subsystem initialized.");
    }

    void Window::ShutdownSubsystem() noexcept
    {
        glfwTerminate();
        MCI_LOG_TRACE("Window subsystem shutdowned.");
    }
